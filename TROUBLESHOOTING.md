...
The Cloudflare Browser Rendering binding is included and enabled by default. The `CDP_SECRET` is auto-generated by `deploy.sh` and stored in 1Password.
...
```
Should return a JSON response with `"Browser": "Cloudflare-Browser-Rendering/..."`.

**If CDP_SECRET is missing** (existing deploys before this was added):
```bash
